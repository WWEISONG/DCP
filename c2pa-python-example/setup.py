...
def read_env_params(env_file_path=None):
  ...
  if run_mode == 'DEV':
      ...
      endpoint_url = app_config.get('AWS_ENDPOINT')
      if not endpoint_url:
          raise KeyError('AWS_ENDPOINT is required when RUN_MODE=DEV')
      
      # Warn if endpoint looks like it's for the wrong service
      if 'sts.' in endpoint_url.lower() and 'kms' not in endpoint_url.lower():
          print(f'WARNING: Endpoint URL appears to be for STS service, not KMS: {endpoint_url}')
          print('  For KMS, the endpoint should be like: https://kms.<region>.amazonaws.com')
          print('  For LocalStack, use: http://localhost:4566')
      
      print(f'Running example setup in dev mode with endpoint: {endpoint_url}')

      # Support both REGION and AWS_REGION for flexibility
      region = app_config.get('AWS_REGION') or app_config.get('REGION')
      if not region:
          raise KeyError('Either AWS_REGION or REGION is required when RUN_MODE=DEV')
...
def create_kms_key(env_file_path=None):
    ...
    try:
      ...
      error_msg = str(e)
      print(f'Error during KMS key creation: {error_msg}')
      
      # Provide helpful error messages for common issues
      if '302' in error_msg or 'redirect' in error_msg.lower():
        print('\nHint: HTTP 302 (redirect) error usually indicates:')
        print('  1. The AWS_ENDPOINT URL is incorrect (e.g., using STS endpoint for KMS)')
        print('  2. For LocalStack, use: http://localhost:4566')
        print('  3. For AWS KMS, use: https://kms.<region>.amazonaws.com')
        print('  4. Ensure the endpoint matches the service (KMS, not STS or other services)')
      elif 'AccessDenied' in error_msg or '403' in error_msg:
        print('\nHint: Access denied - check your AWS credentials and permissions')
      elif 'InvalidParameter' in error_msg:
        print('\nHint: Invalid parameter - check your KeySpec and KeyUsage values')
      
      raise Exception(f'No KMS key id generated (Error during KMS key creation: {error_msg})')
...
def generate_certificate_request(kms_key: str, subject: str, env_file_path=None):
    ...
    try:
        ...
        error_msg = f"Invalid subject format: {subject}"
        print(f"Error parsing certificate subject: {error_msg}")
        print("\nValid attribute types for RFC 4514 format include:")
        print("  CN  - Common Name")
        print("  O   - Organization")
        print("  OU  - Organizational Unit")
        print("  C   - Country (2-letter code)")
        print("  ST  - State or Province")
        print("  L   - Locality (City)")
        print("  E   - Email Address")
        print("  DC  - Domain Component")
        print("  UID - User ID")
        print("\nExample: 'CN=John Smith,O=C2PA Python Demo,C=US'")
        print(f"\nYour input was: '{subject}'")
        raise ValueError(f"Invalid certificate subject format. {error_msg}")
