...
When running `make local` to start the example, the included Python client runs only once. If you wish to re-run the Python client, you can use the make command:

```shell
make client
```

Or run the docker compose command directly (note: you may need `sudo` if you're not in the docker group):

```shell
sudo docker compose run --entrypoint "python tests/client.py ./tests/A.jpg -o client_volume/signed-images" client
```
